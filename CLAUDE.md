...
# Single item baseline regeneration — MUST invoke playwright directly, not via test:visual
...
- **NEVER** pass `-g <name>` / `--update-snapshots` through `pnpm test:visual -- ...`. pnpm appends trailing args to the shell-chained script, so flags don't reach Playwright. Always call `pnpm --filter e2e exec playwright test ...` directly.

Pipeline: `visual.spec.ts` screenshots → `apps/e2e/tests/visual.spec.ts-snapshots/<name>.png` + `<name>-dark.png` → `scripts/sync-og.ts` reads light baselines, passes through `scripts/og-overlay.tsx` (satori + Geist fonts, renders title/install-cmd bar) → writes final OG PNGs to `apps/website/public/og/` (gitignored, rebuilt on deploy).
...
- Interactive registries (need a click/hover before screenshot) register a setup fn in `apps/e2e/tests/visual-setups.ts` keyed by registry name. Runs after page load, before `toHaveScreenshot`.
...
- Run `pnpm --filter e2e exec playwright test --project=visual -g <registry_name> --update-snapshots && pnpm sync:og` → writes baseline to `apps/e2e/tests/visual.spec.ts-snapshots/` + OG to `apps/website/public/og/<registry_name>.png` (the latter is gitignored)
