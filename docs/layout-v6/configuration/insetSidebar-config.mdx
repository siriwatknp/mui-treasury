...
To use `InsetSidebar`, render Inset Sidebar component as the last child to keep it to the right of the content.
...
export function DemoInsetSidebarSticky() {
  return (
    <Root
      sx={{
        ...applyRootStyles({ height: "30vh", fixedHeight: true }),
        overflow: "auto",
      }}
    >
      <Content>
...
export function DemoInsetSidebarAbsolute() {
  return (
    <Root sx={{ ...applyRootStyles({ fixedHeight: true, height: "30vh" }) }}>
      <Content
        sx={{
          minHeight: 0,
        }}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <InsetSidebar
          sx={{
            ...applyInsetSidebarStyles({ position: "fixed", width: "120px" }),
          }}
        >
...
> Note: For nested layout with `fixed` inset sidebar,
> you need to add CSS `container-type: inline-size` to the `Root` component to create boundary for fixed inset sidebar.
