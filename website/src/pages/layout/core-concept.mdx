...
must be sent to `Root` component to let it spread the commands. `Root` is react context provider behind the scene (no dark magic here)
...
```js
// highlight-start
import Layout, { Root, getLayoutComponents } from '@mui-treasury/layout';
...
<PageNavigation prev={{ text: 'Layout introduction', to: '/layout' }} next={{ text: 'Dashboard tutorial', to: '/layout/tutorials/dashboard-layout/' }} />