...
describe('add command integration', () => {
  ...
  it('writes component files to correct target paths', async () => {
    ...
    expect(readFileSync(componentPath, 'utf-8')).toContain(
      'export const button',
    );
  ...
  it('resolves and writes transitive dependencies', async () => {
    ...
    expect(
      existsSync(
        path.join(TEST_DIR, 'src/mui-treasury/components/button/button.tsx'),
      ),
    ).toBe(true);
    expect(
      existsSync(
        path.join(TEST_DIR, 'src/mui-treasury/components/dialog/dialog.tsx'),
      ),
    ).toBe(true);
  });
